...
pub fn main() !u8 {
    ...
    const zigup = try std.fs.path.join(allocator, &.{ test_dir, "bin", "zigup" ++ comptime builtin.target.exeFileExt() });
    ...
    if (builtin.os.tag == .windows) {
        const zigup_src_pdb = try std.mem.concat(allocator, u8, &.{ zigup_src_exe[0 .. zigup_src_exe.len - 4], ".pdb" });
    ...
    // verify that we get an error if there is another compiler in the path
    {
        ...
        {
            const fake_zig = try std.fs.path.join(allocator, &.{ bin2_dir, "zig" ++ comptime builtin.target.exeFileExt() });
