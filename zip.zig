...
pub fn main() !void {
    ...
    for (paths_to_include) |path_ptr| {
        ...
        switch (stat.kind) {
            ...
            .block_device, .character_device, .named_pipe, .unix_domain_socket, .whiteout, .door, .event_port, .unknown => fatal("file '{s}' is an unsupported type {s}", .{ path, @tagName(stat.kind) }),
        }
    }
